...
export async function listWebSmsLogs(req, res) {
  ...
  if (requestedGroupId && req.user.role === "admin") {
    const result = await query(
      "select id, received_at, text_length, text_preview, text, status, asset_id from websms_logs where group_id = $1 order by received_at desc limit $2",
  ...
  if (!logGroupId) {
    ...
    if (status === "processed") {
      ...
      await client.query(
        ...
        [delta, assetId, req.websmsGroupId]
      );
    }
